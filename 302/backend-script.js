...
function submitReportEmail(email, chapter, uniqueCode, reportContent) {
  try {
    ...
    // 2. Also send student confirmation copy with study material
    try {
...
/**
 * OPTIONAL INTERNAL GMAIL AUTO-LOGGER SCRIPT
 * Run this function on a time-driven trigger (e.g. every 10 or 30 minutes)
 * inside Dr. Mora's private Google Apps Script attached to the master sheet.
 * It reads unread completion emails sent to camilo@hawaii.edu and marks them in the spreadsheet.
 */
function processInboxSubmissions() {
  const query = 'subject:"[iStudy Completion]" is:unread';
  ...
  threads.forEach(thread => {
    ...
    messages.forEach(msg => {
      ...
      const studentMatch = body.match(/Student:\s*([^\r\n]+)/i);
      const chapterMatch = body.match(/Chapter:\s*([^\r\n]+)/i);
      const codeMatch = body.match(/Completion Code:\s*([^\r\n]+)/i);

      if (studentMatch && chapterMatch) {
        const studentEmail = studentMatch[1].trim();
        const chNum = parseInt(chapterMatch[1].replace(/[^0-9]/g, ''));
        const padded = (chNum < 10 ? "0" : "") + chNum;
        const uniqueCode = codeMatch ? codeMatch[1].trim() : "✓";

        const chapterColIdx = headers.findIndex(h => {
          const clean = h.toLowerCase().replace(/[^a-z0-9]/g, '');
          return clean === "chapter" + chNum || clean === "chapter" + padded;
        });

        const rowIdx = data.findIndex(row => row[emailIdx] && row[emailIdx].toString().toLowerCase().trim() === studentEmail.toLowerCase());

        if (rowIdx !== -1 && chapterColIdx !== -1) {
          sheet.getRange(rowIdx + 1, chapterColIdx + 1).setValue("✓").setNote(uniqueCode);
        }
      }
      msg.markRead();
    });
